...
#ifndef MPIDI_USE_DYNAMIC_ADI
#include "mpid_request.c"
#include "mpid_init.c"
#include "mpid_win.c"
#include "mpid_progress.c"
#include "mpid_dynproc.c"
#include "mpid_coll.c"
#endif
...
#undef FCNAME
#define FCNAME DECL_FUNC(MPIDI_VCRT_Create)
int MPIDI_VCRT_Create(int size, struct MPIDI_VCRT **vcrt_ptr)
{
  ...
  BEGIN_FUNC(FCNAME);
  vcrt = (struct MPIDI_VCRT *)MPL_malloc(sizeof(struct MPIDI_VCRT) +
...
#undef FCNAME
#define FCNAME DECL_FUNC(MPIDI_VCRT_Release)
...
{
  MPID_THREAD_CS_ENTER(POBJ,MPIR_THREAD_POBJ_UTIL_MUTEX);
  *_map = (void *)MPL_malloc(sizeof(uint64_map));
  new(*_map) uint64_map();
  MPID_THREAD_CS_EXIT(POBJ,MPIR_THREAD_POBJ_UTIL_MUTEX);
}
...
#undef FCNAME
#define FCNAME DECL_FUNC(WinLockAdvance)
static inline int
WinLockAdvance(MPIR_Win *win)
{
...
#undef FCNAME
#define FCNAME DECL_FUNC(WinLockReq_proc)
__SI__ void WinLockReq_proc(const MPIDI_Win_control_t  *info,
                            MPIR_Win                   *win,
                            unsigned                    peer)
{
  BEGIN_FUNC(FCNAME);

  struct MPIDI_Win_lock *lock = (struct MPIDI_Win_lock *)MPL_calloc(1, sizeof(struct MPIDI_Win_lock));
...
#undef FCNAME
#define FCNAME DECL_FUNC(MPIDI_WinLockAck_proc)
__SI__ void WinLockAck_proc(const MPIDI_Win_control_t *info,
                            MPIR_Win                   *win,
                            unsigned                    peer)
{
  BEGIN_FUNC(FCNAME);
...
#undef FCNAME
#define FCNAME DECL_FUNC(WinUnlock_proc)
__SI__ void WinUnlock_proc(const MPIDI_Win_control_t *info,
                           MPIR_Win                   *win,
                           unsigned                    peer)
{
  BEGIN_FUNC(FCNAME);
  --WIN_OFI(win)->sync.lock.local.count;
  MPIR_Assert((int)WIN_OFI(win)->sync.lock.local.count >= 0);
  WinLockAdvance(win);
...
#undef FCNAME
#define FCNAME DECL_FUNC(WinComplete_proc)
__SI__ void WinComplete_proc(const MPIDI_Win_control_t *info,
                             MPIR_Win                   *win,
                             unsigned                    peer)
{
  BEGIN_FUNC(FCNAME);
...
#undef FCNAME
#define FCNAME DECL_FUNC(WinPost_proc)
__SI__ void WinPost_proc(const MPIDI_Win_control_t *info,
                         MPIR_Win                   *win,
                         unsigned                    peer)
{
  BEGIN_FUNC(FCNAME);
...
#undef FCNAME
#define FCNAME DECL_FUNC(WinUnlockDoneCB)
__SI__ void WinUnlockDoneCB(const MPIDI_Win_control_t *info,
                            MPIR_Win                   *win,
                            unsigned                    peer)
{
  BEGIN_FUNC(FCNAME);
  ...
  if(WIN_OFI(win)->sync.origin_epoch_type == MPID_EPOTYPE_LOCK)
    ...
    MPIR_Assert((int)WIN_OFI(win)->sync.lock.remote.allLocked > 0);
    WIN_OFI(win)->sync.lock.remote.allLocked -= 1;
  } else
    MPIR_Assert(0);
...
#undef  FCNAME
#define FCNAME DECL_FUNC(MPIDI_Gethuge_callback)
int MPIDI_Gethuge_callback(cq_tagged_entry_t *wc,
                           MPIR_Request      *req)
{
  int mpi_errno = MPI_SUCCESS;
  BEGIN_FUNC(FCNAME);
...
#undef FCNAME
#define FCNAME DECL_FUNC(MPIDI_Gethuge_cleanup)
__SI__ void MPIDI_Gethuge_cleanup(MPIDI_Send_control_t *info)
{
  BEGIN_FUNC(FCNAME);
  ...
  comm_ptr              = (MPIR_Comm *)MPIDI_Map_lookup(MPIDI_Global.comm_map,mapid);

  /* Look up the per destination receive queue object */
  recv = (MPIDI_Huge_recv_t *)MPIDI_Map_lookup(COMM_OFI(comm_ptr)->huge_recv_counters,
                                               info->origin_rank);
  MPIDI_Map_destroy(recv->chunk_q);
  MPIDI_Map_erase(COMM_OFI(comm_ptr)->huge_recv_counters,info->origin_rank);
  MPL_free(recv);
}

#undef FCNAME
#define FCNAME DECL_FUNC(MPIDI_Gethuge)
__SI__ void MPIDI_Gethuge(MPIDI_Send_control_t *info)
{
  BEGIN_FUNC(FCNAME);
  MPIDI_Huge_recv_t  *recv;
  ...
  MPIR_Comm          *comm_ptr;
  /* Look up the communicator */
  comm_ptr              = (MPIR_Comm *)MPIDI_Map_lookup(MPIDI_Global.comm_map,
                                                        info->comm_id);
  /* Look up the per destination receive queue object */
  recv = (MPIDI_Huge_recv_t *)MPIDI_Map_lookup(COMM_OFI(comm_ptr)->huge_recv_counters,
                                               info->origin_rank);
...
int  MPIDI_control_assert(void *buf)
{
  MPIR_Assert(0);
  return 0;
}
...
#undef FCNAME
#define FCNAME DECL_FUNC(MPIDI_control_callback)
int MPIDI_control_callback(void *buf)
{
  int mpi_errno = MPI_SUCCESS;
  int senderrank;
  BEGIN_FUNC(FCNAME);
  ...
  win        = (MPIR_Win *)MPIDI_Map_lookup(MPIDI_Global.win_map,control->win_id);
  MPIR_Assert(win != MPIDI_MAP_NOT_FOUND);
...
#undef FCNAME
#define FCNAME DECL_FUNC(MPIDI_Control_dispatch)
int MPIDI_Control_dispatch(cq_tagged_entry_t *wc,
                           MPIR_Request      *req)
{
  int mpi_errno = MPI_SUCCESS;
  BEGIN_FUNC(FCNAME);

  MPIR_Assert(wc->len == 0 || wc->len == MPID_MIN_CTRL_MSG_SZ);
  if (wc->len == MPID_MIN_CTRL_MSG_SZ) {
    int16_t *buf, dispatch_id;
    buf         = (int16_t *)wc->buf;
    MPIR_Assert(buf != NULL);
    dispatch_id = *buf;
...
__SI__ int MPIDI_Datatype_to_fi(MPI_Datatype  dt,
                                fi_datatype  *fi_dt,
                                MPI_Op        op,
                                fi_op        *fi_op)
{
...
__SI__ void MPIDI_Create_dt_map()
{
  int i, j;
